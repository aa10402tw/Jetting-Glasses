...
BASE_PATH = os.path.join(os.path.abspath('.').split('Jetting-Glasses')[0], 'Jetting-Glasses\\Aruco\\Camera Calibration\\CalibResult')
...
def loda_camera_params(camera='webcam'):
    if camera == 'webcam':
        file_name = BASE_PATH + "/calibration_webcam.yaml"
    elif(camera == 'usb'):
        file_name = BASE_PATH + "/calibration_usb.yaml"
    elif camera == 'realtime':
        file_name = BASE_PATH + "/calibration_realtime.yaml"
    else:
